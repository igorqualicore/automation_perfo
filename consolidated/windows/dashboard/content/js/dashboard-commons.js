 ...
 * 
 * E.g : duration = 20000100, returns "45 min 20 s 100 ms"
 */
function formatDuration(duration, spaced) {
    ...
    var sonds = Math.floor(duration / 1000); // 1 sond
    duration %= 1000;

    // Add non zero part.
    var formatArray = [];
    if (days > 0)
        formatArray.push(formatUnit(days, " dia(s)", spaced));

    if (hours > 0)
        formatArray.push(formatUnit(hours, " hora(s)", spaced));

    if (minutes > 0)
        formatArray.push(formatUnit(minutes," min", spaced));

    if (seconds > 0)
        formatArray.push(formatUnit(seconds, " s", spaced));
...
function getElapsedTimeLabel(granularity) {
    return "Tempo decorrido (granularidade: " + formatDuration(granularity) + ")";
}
...
function getConnectTimeLabel(granularity) {
    return "Tempo de conexao (granularidade: " + formatDuration(granularity) + ")";
}
